...
import '../HouseSell/housesell.css';
...
const HouseLease = () => {
    ...
    const callApi =async()=>{
        setIsLoading(true);
         ...
         if(response.status === 200){
             setRentList(response.data.filter(d => d.sale_or_rent =='rent'))
         }
  ...
  return (
    ...
    <div className='main-housesell'>
      <h3>Cho thuê nhà đất trên toàn quốc</h3>
      <p>Hiện có {rentList.filter(d=>d.status==true).length} bất động sản</p><br/>
      { rentList.length >0 ? 
